...
function searchShowtimes() {
	...
	var zip = $('#zip').val();
	if ($('#search').val() != "") {
		alert("why the fuck am i in here");
		var request = $.ajax({
		...
		stError("Please enter a movie first.");
	}
}
...
function addShowtimes(movies, movie) {
	$('.showtimes').empty();
	var query = movie;
	console.log(query + "is the search");
	for (var i = 0; i < movies.length; i++) {
